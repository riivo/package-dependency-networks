...
if __name__ == "__main__":
    ...
    files = glob.glob("../data/results_path/paths-ver-{0}Exp-ts-*rev.tsv".format(lang))
    random.shuffle(files)
    print "Files", len(files)    
    summary =  Parallel(n_jobs=14, verbose=51)(delayed(collect)(i) for i in files)
        
    summary = pd.concat(summary)    
    
    summary.to_csv("../data/results_path/agg_path_{0}.csv".format(lang))
    #%%
    
    #summary.boxplot(by="date")
